...
// Design option B: `ProofEngine` is `dyn`-compatible now that opt-out is
// a method (`is_null`) rather than an associated const, so the task holds
// `Arc<dyn ProofEngine<P> + Send + Sync>` directly. No `ProofVerifier`
// facade, no second engine generic on `Controller`/`ThreadPool`; the full
// `validate_execution_proof_gossip` pipeline will call through this
// handle in place.
pub struct ProcessExecutionProofTask<P: Preset, W> {
    pub store_snapshot: Arc<Store<P, Storage<P>>>,
    pub proof_engine: Arc<dyn ProofEngine<P> + Send + Sync>,
    pub mutator_tx: Sender<MutatorMessage<P, W>>,
...
mod tests {
    ...
    fn run_stub_task(
        proof_engine: Arc<dyn proof_engine::ProofEngine<Minimal> + Send + Sync>,
    ...
    fn stub_pipeline_ignores_against_mock_engine() {
        for execution_proof_valid in [true, false] {
            let proof_engine: Arc<dyn proof_engine::ProofEngine<Minimal> + Send + Sync> =
