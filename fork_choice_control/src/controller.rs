    ...
    W: Wait,
{
    ...
    // Nothing calls this until gossip wiring lands. The generic `PE` is erased
    // into `Arc<dyn ProofVerifier<P>>` here so `Controller` and `ThreadPool`
    // stay free of a second engine generic.
    #[expect(dead_code)]
